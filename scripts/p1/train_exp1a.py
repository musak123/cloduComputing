...
def main() -> None:
    ...
    tracker = EmissionTrackerFacade(project_name="p1_train_exp1a", output_dir=joint_cfg.output_dir)
    tracker.start()
    ...
    summary["emissions"] = {"provider": track.provider, "duration_s": round(track.duration_s,4), "energy_kwh": round(track.energy_kwh,8), "co2_kg": round(track.co2_kg,8)}
    summary["recommendations"] = reduction_recommendations()
    out_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    write_emission_row(Path(joint_cfg.output_dir) / "emissions_tracking.csv", {"project": "p1_train_exp1a", "provider": track.provider, "duration_s": round(track.duration_s,4), "energy_kwh": round(track.energy_kwh,8), "co2_kg": round(track.co2_kg,8)})
